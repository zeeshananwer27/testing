...
<div class="col-lg-8 col-md-7 col-sm-7 col-xs-12">
    ...
    <!--        <span class="out-of-stock">{{ trans('storefront::product.out_of_stock') }}</span>-->
        ...
        <div class="col-sm-12 no-padding">
            ...
            <!--<button type="submit" class="add-to-cart btn btn-primary pull-left btn-size" {{ $product->isOutOfStock() ? 'disabled' : '' }} data-loading>-->
                <input type="hidden" id="seats_data" class="seats_data" name="seats_data">
                <div id="diagram">
    ...
    <script type="text/javascript">
        ...
        $(document).on('click','#add_to_cart',function(e) {
            ...
            document.getElementById("seats_data").value=JSON.stringify(a);
            }else{
            $('.seats_data').val('');
            }
            ...
            var sitting_check = <?php echo $sitting_check; ?>;
            for (i=0;i<advancedOptions.length;i++){
                ad= document.getElementById("ad_"+advancedOptions[i]['id']).value;
                t_qty +=parseInt(ad);
            }
            if(data1.length != 0){
            $('.showall').hide();
            ...
            }else if(sitting_check==0 && t_qty>0){
                $('.seats_data').val('');
                  $.ajax({
            type: 'post',
            url: "{{ route('cart.items.ajaxstore') }}",
    ...
    <script >
       ...
       @else
            dataDiagram = <?php echo $product['sitting_arrangement']; ?>;
       @endif
        var advancedOptions = <?php echo json_encode($advancedOptions); ?>;
        ...
        @if(count($reservedSeats) != 0 )
            ...
            <?php echo $reservedSeats ?>.map(item => {
        ...
        function getValues(){
            ...
            document.getElementById("seats_data").value=JSON.stringify(selectedSeats);
            for(i=0;i<id; i++)
            {
                var value=0;
                ...
                for (j=0;j<selectedSeats.length;j++)
                {
                    if(advancedOptions[i]['id']==selectedSeats[j]['advancedOptionId']){
                        ...
                        limit= document.getElementById("limit_"+advancedOptions[i]['id']).value;
                        document.getElementById(advancedOptions[i]['id']).innerHTML =  parseInt(limit)-value;
                    }

                }
                if (selectedSeats.length==0){
                    limit= document.getElementById("limit_"+advancedOptions[i]['id']).value;
                    document.getElementById(advancedOptions[i]['id']).innerHTML =  limit;
                }
            }
