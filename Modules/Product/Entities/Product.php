...
class Product extends Model
{
     ...
     * @var array
     */
    // protected $translatedAttributes = ['name', 'description','short_description', 'city', 'state', 'zip_code'];
    protected $translatedAttributes = ['name', 'description','organizer',  'short_description', 'address', 'city', 'state', 'zip_code', 'lat', 'lng'];
    ...
    {
        $query->with('translations:id,product_id,locale,name,city,state,start_event');
    }
    ...
    {
        return $this->hasOne('Modules\Product\Entities\ProductTranslation');
    }
    ...
    public static function findBySlug_rist($slug)
    {
        echo $slug; 
        $abc =  static::with([
            'attributes.attribute.attributeSet', 'options', 'files', 'relatedProducts', 'upSellProducts',
        ...
        echo "<pre>"; print_r($abc); exit('suna');  
    ...
    {
        return $this->belongsToMany(User::class, 'user_events', 'product_id', 'user_id')->withTimestamps();
    }
