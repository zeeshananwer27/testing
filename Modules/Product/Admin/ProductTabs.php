...
class ProductTabs extends Tabs
{
    public function make()
    {
        ...
        // $this->group('Settings', trans('product::products.tabs.group.settings'))
        
        $this->group('Settings', "Sitting arrangement")
            ->add($this->sitting_arrangement());
    }
    ...
    private function general()
    {
        return tap(new Tab('general', trans('product::products.tabs.general')), function (Tab $tab) {
            $tab->active();
            $tab->weight(5);
            $tab->fields(['name', 'description','organizer', 'address','lat', 'lng','start_event', 'end_event',  'is_active']);
    ...
    private function additionals()
    {
     ...
     private function sitting_arrangement()
    {
        return tap(new Tab('Arrangement', 'Arrangement'), function (Tab $tab) {
            $tab->weight(55);
            // $tab->fields(['new_from', 'new_to']);
            $tab->view('product::admin.products.sitting_arrangement');
        });
    }
    private function advancedOptions()
    {
        return tap(new Tab('advanced_options', trans('product::products.tabs.advanced_options')), function (Tab $tab) {
            $tab->weight(15);
            $tab->fields(['advanced_label.*', 'advanced_price.*', 'advanced_quantity.*', 'purchase_limit.*']);
            $tab->view('product::admin.products.tabs.advanced_options', [
