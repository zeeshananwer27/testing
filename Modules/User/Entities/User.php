...
class User extends EloquentUser implements AuthenticatableContract
{
    ...
    {
        return $this->belongsToMany(Product::class, 'user_events')->withoutGlobalScope('active')->withTimestamps();
    }    
    ...
    public static function list($ids = '')
    {
        ...
        if(!empty($ids)){
            return static::where('product_id', $ids)
            ...
            $e = static::select('id', 'first_name', 'last_name')->whereHas('roles', function ($query) use ($re) {return $query->whereIn('id',  $re); })
    ...
    public static function adminStaffList($ids =''){
        $customer=Role::adminAndStaff();
        if($customer){
            return static::select('id', 'first_name', 'last_name')->whereHas('roles', function ($query) use ($customer) {return $query->whereIn('id',  $customer); })
                    ->get()->mapWithKeys(function ($user) {
                    return [$user->id => $user->first_name.' '.  $user->last_name];
                });            
        }else{
