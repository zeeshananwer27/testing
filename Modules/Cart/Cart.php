...
class Cart extends DarryldecodeCart
{
     ...
     * @return $this
     */
    public function store($productId, $qty, $options = [], $data)
    {



        // echo "<pre>"; print_r($data); exit('blew');

        $product = Product::with('files', 'categories', 'taxClass')->findOrFail($productId);
        $options = array_filter($options);

        $advance_options = $data;



        $total_price = 0;
        foreach ($advance_options['quantity'] as $key => $value) {
            ...
            foreach ($value as $key1 => $val) {
                ...
                if ($val != 0 && $val !='') {
                    ...
                    if(is_numeric($key1)){
                        ...
                        if(!empty($advance_options['fee'][$key])){
                            $total_price += $advance_options['fee'][$key] * $val ;
                        }
...
//        dd($total_price);
//        $total_price=789;
        $s = $this->options($product, $options);
        return $this->add([
    ...
    public function updateAdvanceOption($id, $qty)
    {
        $this->update($id, [
            'quantity' => [
                'relative' => false,
    ...
    {
       if (! is_null($this->advance_options)) {
    ...
    public function reduceStock()
    {
        $this->manageStock(function ($cartItem) {
            ...
            if ($cartItem->product->fresh()->qty === 0) {
